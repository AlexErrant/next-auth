...
export async function callback(params) {
    ...
    try {
        if (provider.type === "oauth" || provider.type === "oidc") {
            ...
            const unauthorizedOrError = await handleAuthorized({ user: userOrProfile, account, profile: OAuthProfile, signinInfo }, options);
            if (unauthorizedOrError)
                return { ...unauthorizedOrError, cookies };
            // Sign user in
            const { user, session, isNewUser } = await handleLogin(sessionStore.value, profile, account, options, signinInfo);
            if (useJwtSession) {
                const defaultToken = {
                    name: user.name,
                    email: user.email,
            ...
            // Check if user is allowed to sign in
            const unauthorizedOrError = await handleAuthorized({ user, account, signinInfo }, options);
            if (unauthorizedOrError)
                return { ...unauthorizedOrError, cookies };
            // Sign user in
            const { user: loggedInUser, session, isNewUser, } = await handleLogin(sessionStore.value, user, account, options, signinInfo);
            if (useJwtSession) {
                const defaultToken = {
            ...
            const unauthorizedOrError = await handleAuthorized({ user, account, credentials, signinInfo }, options);
            if (unauthorizedOrError)
                return { ...unauthorizedOrError, cookies };
            const defaultToken = {
                name: user.name,
